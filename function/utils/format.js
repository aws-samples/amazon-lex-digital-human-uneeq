...
let responseJSON = (answer, instructions, conversationPayload) => {
    ...
    console.debug(`Response JSON: ${responsePayload}`)
...
let parseInstructions = (response) => {
    ...
    try {
        ...
        if (responseInstructions.hasOwnProperty('displayHtml')) {
            if (typeof responseInstructions.displayHtml == 'string') {
                displayHtml = JSON.parse(responseInstructions.displayHtml)
            } else {
                displayHtml = responseInstructions.displayHtml
            }
            if (displayHtml.hasOwnProperty('html')) {
                instructions.displayHtml = displayHtml
                console.debug(`Parsed a displayHtml payload: ${JSON.stringify(displayHtml)}`)
            } else {
                console.warn("A displayHtml instruction was passed without the required 'html' field")
            }
        }
        ...
        if (responseInstructions.hasOwnProperty('extraHintPhrases')) {
            ...
            if (extraHintPhrases.hasOwnProperty('phrases')) {
                if (Array.isArray(extraHintPhrases.phrases)) {
                    ...
                    console.debug(`Parsed a extraHintPhrases payload: ${JSON.stringify(extraHintPhrases)}`)
                } else {
                    console.warn("The 'phrases' parameter in extraHintPhrases did not contain an array")
                }
            } else {
                console.warn("An extraHintPhrases instruction was passed without the required 'phrases' field")
            }
        }
        ...
        if (responseInstructions.hasOwnProperty('privateDataFilter')) {
            ...
            if (privateDataFilter.hasOwnProperty('enabled')) {
                if (typeof privateDataFilter.enabled == 'boolean') {
                    ...
                    console.debug(`Parsed a privateDataFilter payload: ${JSON.stringify(privateDataFilter)}`)
                } else {
                    console.warn("The 'enabled' parameter in privateDataFilter did not contain a boolean")
                }
            } else {
                console.warn("A privateDataFilter instruction was passed without the required 'enabled' field")
            }
        }
        ...
        console.warn(`Could not parse instructions parameter: ${response}`)
        console.warn(error)
    }
...
let parseAnswer = async (text) => {
    ...
    try {
        ...
        console.error(`The answer response contained structurally invalid markup: ${text}`)
        throw error
    }
}
